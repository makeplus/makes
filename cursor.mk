...
ifdef CURSOR-VERSION
  CURSOR-DATE := $(shell \
    curl -sL $(CURSOR-HISTORY-URL) | jq -r '.versions[0].date')
  URL-linux-int64 := $(shell \
    curl -sL $(CURSOR-HISTORY-URL) | \
    jq -r '.versions[] | \
      select(.version == "$(CURSOR-VERSION)") | \
      .platforms."linux-x64"')
else
  CURSOR-VERSION := 1.2.2
  CURSOR-DATE := 2025-07-07
  URL-linux-int64 := \
    https://downloads.cursor.com/production/faa03b17cce93e8a80b7d62d57f5eda6bb6ab9fa/linux/x64/Cursor-1.2.2-x86_64.AppImage
endif
