...
OA-linux-int64 := ubuntu-22.04-x86_64
...
FREEBASIC-DIR := Binaries-Windows
FREEBASIC := $(FREEBASIC-LOCAL)/fbc64.exe
else
FREEBASIC-ARCHIVE := FreeBASIC-$(FREEBASIC-VERSION)-$(OA-$(OS-ARCH)).tar.xz
FREEBASIC-DIR := Binaries-Linux
FREEBASIC := $(FREEBASIC-LOCAL)/bin/fbc
endif
FREEBASIC-DOWN := https://sourceforge.net/projects/fbc/files/FreeBASIC-$(FREEBASIC-VERSION)/$(FREEBASIC-DIR)/$(FREEBASIC-ARCHIVE)/download
...
$(FREEBASIC): $(LOCAL-CACHE)/$(FREEBASIC-ARCHIVE)
	...
	$Q rm -rf $(FREEBASIC-LOCAL) $(LOCAL-TMP)/freebasic-$(FREEBASIC-VERSION)
	$Q mkdir -p $(LOCAL-TMP)/freebasic-$(FREEBASIC-VERSION)
	$Q tar -C $(LOCAL-TMP)/freebasic-$(FREEBASIC-VERSION) -xJf $<
	$Q mv $$(find $(LOCAL-TMP)/freebasic-$(FREEBASIC-VERSION) -mindepth 1 -maxdepth 1 -type d | head -1) $(FREEBASIC-LOCAL)
	$Q touch $@
	@$(ECHO)
endif
